...
#define KM_BOOL_DIFF(a, b) (long double)(!(a) != !(b))
...
static inline void
do_pairwise_1 (void *data, cell_t *cells, size_t count, cell_mode_t mode,
        ...
        if (rrr == 0) {
            if (sample_names != NULL) {
                ...
                for (ccc = 0; ccc < mat->samples; ccc++) {
                    fprintf(tab->outfp, "%s\t", sample_names[ccc]);
                }
                fprintf(tab->outfp, "\n");
            }
        }
        if (sample_names != NULL) {
            fprintf(tab->outfp, "%s\t", sample_names[rrr]);
        }
        for (ccc = 0; ccc < mat->samples; ccc++) {
